...
fun <E> FunctionApprox.`n-step semi-gradient off-policy sarsa episodic`(n: Int, b: Policy, q: ApproximateFunction<E>, trans: (State, Action<State>) -> E) {
    ...
    for (episode in 1..episodes) {
        ...
        do {
            ...
            if (τ >= 0) {
                ...
                q.w += α * ρ * (G - q(trans(_S[0], _A[0]))) * q.`▽`(trans(_S[0], _A[0]))
            }
            t++
        } while (τ < T - 1)
        log.debug { "n=$n,T=$T" }
...
fun <E> FunctionApprox.`n-step semi-gradient off-policy sarsa continuing`(n: Int, b: Policy, β: Double, q: ApproximateFunction<E>, trans: (State, Action<State>) -> E) {
