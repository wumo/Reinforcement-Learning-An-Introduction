...
typealias PrioritizedSweepingSE = PrioritizedSweepingStochasticEnvironment
@Suppress("NAME_SHADOWING")
class PrioritizedSweepingStochasticEnvironment(val mdp: MDP) {
    companion object {
    ...
    fun optimal(_alpha: (State, Action) -> Double = { _, _ -> alpha }): OptimalSolution {
        ...
        for (episode in 1..episodes) {
            ...
            while (s.isNotTerminal()) {
                ...
                val P = FastMath.abs(reward + gamma * max(s_next.actions, 0.0) { Q[s_next, it] } - Q[s, a])
                if (P > theta) PQueue.add(tuple3(P, s, a))
                lab.mars.rl.util.repeat(n, { PQueue.isNotEmpty() }) {
                    ...
                    for ((s_pre, a_pre) in predecessor[s]) {
                        ...
                        assert(s_next === s)
                        val P = FastMath.abs(reward + gamma * max(s.actions, 0.0) { Q[s, it] } - Q[s_pre, a_pre])
